...
docker ps -a | grep 'api-di-test' | awk '{print $1}' | xargs --no-run-if-empty docker stop
docker ps -a | grep 'api-di-test' | awk '{print $1}' | xargs --no-run-if-empty docker rm

#Remove existing images if any
docker images | grep 'tilaks/dataingestortest' | awk '{print $3}' | xargs --no-run-if-empty docker rmi -f
