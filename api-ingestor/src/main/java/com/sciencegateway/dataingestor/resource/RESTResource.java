...
public class RESTResource 
{
	...
	public Response generateURL(URLObjects urlObjects)	
	{
		...
		try 
		{
			...
			ClientConfig clientConfigU = new ClientConfig();
			Client clientU = ClientBuilder.newClient(clientConfigU);
			clientU.property(ClientProperties.CONNECT_TIMEOUT, 5000);
			WebTarget targetU = clientU.target("http://127.0.0.1:8080/dataingestor/webapi/service/log");
			System.out.println(targetU.toString());
			Response responseToU = targetU.request().post(Entity.entity(jsonObject.toString(), "application/json"),Response.class);
			System.out.println(responseToU.toString());
			responseToU.close();
	...
	public void generateLOG(URLObjects urlObjects) throws Exception
	{
		...
		WebTarget targetR = clientR.target("http://127.0.0.1:8080/registry/v1/service/log");
